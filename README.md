...
### Setting up LLM
Please start with setting up a first LLM. This can be a local one or a hosted one. For example, when using OpenRouter, go to "LLMs", click on "Add LLM" and enter a title for your LLM (custom label), the model name, e.g. "openai/gpt-5.2", the URL to the api, e.g., "https://openrouter.ai/api/v1" and your API token. Click on save changes. Caution: You need to restart the llm_server after changing your credentials (run "python llm_server.py" or [re]start the WSGI server).

### Starting the Integration
You can now start the JHS integration process by uploading your dataset. Your dataset should contain several products with an ID, title, description text(s) and some possible filters. The dataset can be CSV, XLSX or JSON. Click on "upload and process". 
